...
template <typename CharType = char, CharType end_token = 1000000>
class SuffixTree {
...
public:
    ...
    void dfs(Node *n, boost::dynamic_bitset<> &already_covered_strings, MappedSubstring orig, std::vector<uint64_t> current_path, std::vector<std::vector<uint64_t>> &common_patterns, std::vector<boost::dynamic_bitset<>> &bitmaps)
    {
        if(!orig.empty())
        {
            const auto& s = haystack.find(orig.ref_str);
            for (index_type i = orig.l; i <= orig.r && i < s->second.size(); ++i)
            {
                auto current_val = s->second[i];
                current_path.push_back(current_val);
                if(current_val>=0 && current_val < total_added_strings)
                {
                    already_covered_strings[current_val]=1;
                }
            }
        }
        for(auto t_it:n->g)
        {
            boost::dynamic_bitset<> already_covered_from_child(total_added_strings);
            dfs(t_it.second.tgt, already_covered_from_child, t_it.second.sub, current_path, common_patterns, bitmaps);
            for(boost::dynamic_bitset<>::size_type i=0; i<total_added_strings; i++)
            {
                if(already_covered_from_child[i])already_covered_strings[i]=1;
            }
        }
        if(n->g.size()>0 && current_path.size() > 0)
        {
            common_patterns.push_back(current_path);
            bitmaps.push_back(already_covered_strings);
        }
    }
    void dfs_visit(std::vector<std::vector<uint64_t>> &common_patterns, std::vector<boost::dynamic_bitset<>> &bitmaps)
    {
        boost::dynamic_bitset<> covered_string_bitmap(total_added_strings);
        dfs(&tree.root, covered_string_bitmap, MappedSubstring(0,0,-1), std::vector<uint64_t>(), common_patterns, bitmaps);
    }
};
